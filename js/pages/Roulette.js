...
export default {
   ...
   async mounted() { // Changed to async
      store.rouletteContext = this;
      await resetRoulette();
   },
   computed: {
      currentLevel() {
...
export async function resetRoulette() {
   if (!store.rouletteContext) return;
   store.rouletteContext.loading = true;
   // Create File Input
   store.rouletteContext.fileInput = document.createElement('input');
   store.rouletteContext.fileInput.type = 'file';
   store.rouletteContext.fileInput.multiple = false;
   store.rouletteContext.fileInput.accept = '.json';
   store.rouletteContext.fileInput.addEventListener('change', store.rouletteContext.onImportUpload);

   // Load progress from local storage
   const roulette = JSON.parse(localStorage.getItem('roulette'));

   if (!roulette) {
      store.rouletteContext.loading = false;
      return;
   }

   store.rouletteContext.levels = roulette.levels;
   store.rouletteContext.progression = roulette.progression;
